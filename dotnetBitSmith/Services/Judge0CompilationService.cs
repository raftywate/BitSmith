...
void print_tree_node(struct TreeNode* root) {
    ...
    free(queue);
        ...
        private static string WrapCsharp(string userCode, string methodName, List<string> paramTypes, string returnType) {
            ...
            sb.AppendLine("        var arr = SimpleJson.Deserialize<List<int>>(s);");
            ...
            sb.AppendLine("        if (string.IsNullOrEmpty(s) || s.Trim() == \"null\") return null;");
            sb.AppendLine("        var array = SimpleJson.Deserialize<List<string>>(s);");
            sb.AppendLine("        if (array == null || array.Count == 0) return null;");
            sb.AppendLine("        if (array[0] == \"null\" || string.IsNullOrEmpty(array[0])) return null;");
            sb.AppendLine("        TreeNode root = new TreeNode(int.Parse(array[0]));");
            sb.AppendLine("        Queue<TreeNode> q = new Queue<TreeNode>();");
            sb.AppendLine("        q.Enqueue(root);");
            sb.AppendLine("        int i = 1;");
            sb.AppendLine("        int len = array.Count;");
            ...
            sb.AppendLine("                if (val != \"null\" && !string.IsNullOrEmpty(val)) {");
            sb.AppendLine("                    curr.left = new TreeNode(int.Parse(val));");
            ...
            sb.AppendLine("                var val = array[i];");
            sb.AppendLine("                if (val != \"null\" && !string.IsNullOrEmpty(val)) {");
            sb.AppendLine("                    curr.right = new TreeNode(int.Parse(val));");
            ...
            sb.AppendLine("        if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>)) {");
            sb.AppendLine("            if (string.IsNullOrEmpty(json) || json == \"null\") return null;");
            sb.AppendLine("            var underlyingType = Nullable.GetUnderlyingType(type);");
            sb.AppendLine("            return DeserializeValue(json, underlyingType);");
            sb.AppendLine("        }");
            sb.AppendLine("        if (type == typeof(int)) return int.Parse(json);");
            sb.AppendLine("        if (type == typeof(long)) return long.Parse(json);");
            sb.AppendLine("        if (type == typeof(double)) return double.Parse(json);");
            sb.AppendLine("        if (type == typeof(float)) return float.Parse(json);");
            sb.AppendLine("        if (type == typeof(bool)) return bool.Parse(json);");
            sb.AppendLine("        if (type == typeof(char)) return json.Trim('\\'', '\"')[0];");
            sb.AppendLine("        if (type == typeof(string)) {");
            sb.AppendLine("            if (json.StartsWith(\"\\\"\") && json.EndsWith(\"\\\"\")) {");
            sb.AppendLine("                return json.Substring(1, json.Length - 2);");
            sb.AppendLine("            }");
            sb.AppendLine("            return json;");
            sb.AppendLine("        }");
            sb.AppendLine("        if (type.IsArray) {");
            sb.AppendLine("            var elemType = type.GetElementType();");
            sb.AppendLine("            var list = ParseJsonArray(json);");
            sb.AppendLine("            var arr = Array.CreateInstance(elemType, list.Count);");
            sb.AppendLine("            for (int i = 0; i < list.Count; i++) {");
            sb.AppendLine("                arr.SetValue(DeserializeValue(list[i], elemType), i);");
            sb.AppendLine("            }");
            ...
            sb.AppendLine("            for (int i = 0; i < list.Count; i++) {");
            sb.AppendLine("                listInstance.Add(DeserializeValue(list[i], elemType));");
            sb.AppendLine("            }");
            sb.AppendLine("            return listInstance;");
            sb.AppendLine("        }");
            ...
            sb.AppendLine("        json = json.Trim();");
            sb.AppendLine("        if (!json.StartsWith(\"[\") || !json.EndsWith(\"]\")) return result;");
            sb.AppendLine("        json = json.Substring(1, json.Length - 2).Trim();");
            sb.AppendLine("        if (string.IsNullOrEmpty(json)) return result;");
            sb.AppendLine();
            sb.AppendLine("        int braceCount = 0;");
            sb.AppendLine("        bool inQuotes = false;");
            sb.AppendLine("        int start = 0;");
            sb.AppendLine("        for (int i = 0; i < json.Length; i++) {");
            sb.AppendLine("            char c = json[i];");
            sb.AppendLine("            if (c == '\"' && (i == 0 || json[i-1] != '\\\\')) {");
            sb.AppendLine("                inQuotes = !inQuotes;");
            sb.AppendLine("            }");
            sb.AppendLine("            if (!inQuotes) {");
            sb.AppendLine("                if (c == '[' || c == '{') braceCount++;");
            sb.AppendLine("                else if (c == ']' || c == '}') braceCount--;");
            sb.AppendLine("                else if (c == ',' && braceCount == 0) {");
            sb.AppendLine("                    result.Add(json.Substring(start, i - start).Trim());");
            sb.AppendLine("                    start = i + 1;");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("        result.Add(json.Substring(start).Trim());");
            sb.AppendLine("        return result;");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    public static string Serialize(object obj) {");
            sb.AppendLine("        if (obj == null) return \"null\";");
            sb.AppendLine("        if (obj is string) return \"\\\"\" + obj.ToString().Replace(\"\\\"\", \"\\\\\\\"\") + \"\\\"\";");
            sb.AppendLine("        if (obj is bool) return obj.ToString().ToLower();");
            sb.AppendLine("        if (obj is int || obj is long || obj is double || obj is float || obj is char) return obj.ToString();");
            sb.AppendLine("        ");
            sb.AppendLine("        if (obj is System.Collections.IEnumerable enumerable) {");
            sb.AppendLine("            var list = new List<string>();");
            sb.AppendLine("            foreach (var item in enumerable) {");
            sb.AppendLine("                list.Add(Serialize(item));");
            sb.AppendLine("            }");
            sb.AppendLine("            return \"[\" + string.Join(\",\", list) + \"]\";");
            sb.AppendLine("        }");
            sb.AppendLine("        return obj.ToString();");
            sb.AppendLine("    }");
            sb.AppendLine("}");
            return sb.ToString();
        }
        ...
        private static string WrapJava(string userCode, string methodName, List<string> paramTypes, string returnType) {
            ...
            sb.AppendLine("    private static String trimStr(String s) {");
            sb.AppendLine("        if (s == null) return \"\";");
            sb.AppendLine("        return s.trim();");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    private static String parseString(String s) {");
            sb.AppendLine("        s = trimStr(s);");
            sb.AppendLine("        if (s.startsWith(\"\\\"\") && s.endsWith(\"\\\"\") && s.length() >= 2) {");
            sb.AppendLine("            return s.substring(1, s.length() - 1);");
            sb.AppendLine("        }");
            sb.AppendLine("        return s;");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    private static char parseChar(String s) {");
            sb.AppendLine("        String str = parseString(s);");
            sb.AppendLine("        return str.isEmpty() ? ' ' : str.charAt(0);");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    private static ListNode parseListNode(String s) {");
            sb.AppendLine("        s = trimStr(s);");
            sb.AppendLine("        if (s.isEmpty() || s.equals(\"null\") || s.equals(\"[]\")) return null;");
            sb.AppendLine("        if (s.startsWith(\"[\") && s.endsWith(\"]\")) s = s.substring(1, s.length() - 1);");
            sb.AppendLine("        String[] parts = s.split(\",\");");
            sb.AppendLine("        ListNode dummy = new ListNode(0);");
            sb.AppendLine("        ListNode curr = dummy;");
            sb.AppendLine("        for (String p : parts) {");
            sb.AppendLine("            String t = p.trim();");
            sb.AppendLine("            if (!t.isEmpty()) {");
            sb.AppendLine("                curr.next = new ListNode(Integer.parseInt(t));");
            sb.AppendLine("                curr = curr.next;");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine("        return dummy.next;");
            ...
            sb.AppendLine("        if (s.startsWith(\"[\") && s.endsWith(\"]\")) s = s.substring(1, s.length() - 1).trim();");
            sb.AppendLine("        if (s.isEmpty()) return new String[0];");
            sb.AppendLine("        List<String> list = new ArrayList<>();");
            sb.AppendLine("        int depth = 0;");
            ...
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    private static Object parseArg(String raw, Class<?> clazz, Type genericType) {");
            sb.AppendLine("        raw = trimStr(raw);");
            sb.AppendLine("        if (clazz == int.class || clazz == Integer.class) return Integer.parseInt(raw);");
            sb.AppendLine("        if (clazz == long.class || clazz == Long.class) return Long.parseLong(raw);");
            sb.AppendLine("        if (clazz == double.class || clazz == Double.class) return Double.parseDouble(raw);");
            sb.AppendLine("        if (clazz == float.class || clazz == Float.class) return Float.parseFloat(raw);");
            sb.AppendLine("        if (clazz == boolean.class || clazz == Boolean.class) return Boolean.parseBoolean(raw);");
            sb.AppendLine("        if (clazz == char.class || clazz == Character.class) return parseChar(raw);");
            sb.AppendLine("        if (clazz == String.class) return parseString(raw);");
            sb.AppendLine("        if (clazz == ListNode.class) return parseListNode(raw);");
            sb.AppendLine("        if (clazz == TreeNode.class) return parseTreeNode(raw);");
            sb.AppendLine();
            sb.AppendLine("        if (clazz.isArray()) {");
            sb.AppendLine("            Class<?> comp = clazz.getComponentType();");
            sb.AppendLine("            String[] parts = splitJsonArray(raw);");
            sb.AppendLine("            if (comp == int.class) {");
            sb.AppendLine("                int[] arr = new int[parts.length];");
            sb.AppendLine("                for (int i = 0; i < parts.length; i++) arr[i] = Integer.parseInt(parts[i].trim());");
            sb.AppendLine("                return arr;");
            sb.AppendLine("            }");
            sb.AppendLine("            if (comp == long.class) {");
            sb.AppendLine("                long[] arr = new long[parts.length];");
            sb.AppendLine("                for (int i = 0; i < parts.length; i++) arr[i] = Long.parseLong(parts[i].trim());");
            sb.AppendLine("                return arr;");
            sb.AppendLine("            }");
            sb.AppendLine("            if (comp == double.class) {");
            sb.AppendLine("                double[] arr = new double[parts.length];");
            sb.AppendLine("                for (int i = 0; i < parts.length; i++) arr[i] = Double.parseDouble(parts[i].trim());");
            sb.AppendLine("                return arr;");
            sb.AppendLine("            }");
            sb.AppendLine("            if (comp == boolean.class) {");
            sb.AppendLine("                boolean[] arr = new boolean[parts.length];");
            sb.AppendLine("                for (int i = 0; i < parts.length; i++) arr[i] = Boolean.parseBoolean(parts[i].trim());");
            sb.AppendLine("                return arr;");
            sb.AppendLine("            }");
            sb.AppendLine("            if (comp == char.class) {");
            sb.AppendLine("                char[] arr = new char[parts.length];");
            sb.AppendLine("                for (int i = 0; i < parts.length; i++) arr[i] = parseChar(parts[i]);");
            sb.AppendLine("                return arr;");
            sb.AppendLine("            }");
            sb.AppendLine("            if (comp == String.class) {");
            sb.AppendLine("                String[] arr = new String[parts.length];");
            sb.AppendLine("                for (int i = 0; i < parts.length; i++) arr[i] = parseString(parts[i]);");
            sb.AppendLine("                return arr;");
            sb.AppendLine("            }");
            sb.AppendLine("            if (comp.isArray()) {");
            sb.AppendLine("                Class<?> subComp = comp.getComponentType();");
            sb.AppendLine("                if (subComp == int.class) {");
            sb.AppendLine("                    int[][] mat = new int[parts.length][];");
            sb.AppendLine("                    for (int i = 0; i < parts.length; i++) {");
            sb.AppendLine("                        String[] subParts = splitJsonArray(parts[i]);");
            sb.AppendLine("                        mat[i] = new int[subParts.length];");
            sb.AppendLine("                        for (int j = 0; j < subParts.length; j++) mat[i][j] = Integer.parseInt(subParts[j].trim());");
            sb.AppendLine("                    }");
            sb.AppendLine("                    return mat;");
            sb.AppendLine("                }");
            sb.AppendLine("                if (subComp == char.class) {");
            sb.AppendLine("                    char[][] mat = new char[parts.length][];");
            sb.AppendLine("                    for (int i = 0; i < parts.length; i++) {");
            sb.AppendLine("                        String[] subParts = splitJsonArray(parts[i]);");
            sb.AppendLine("                        mat[i] = new char[subParts.length];");
            sb.AppendLine("                        for (int j = 0; j < subParts.length; j++) mat[i][j] = parseChar(subParts[j]);");
            sb.AppendLine("                    }");
            sb.AppendLine("                    return mat;");
            sb.AppendLine("                }");
            sb.AppendLine("                if (subComp == String.class) {");
            sb.AppendLine("                    String[][] mat = new String[parts.length][];");
            sb.AppendLine("                    for (int i = 0; i < parts.length; i++) {");
            sb.AppendLine("                        String[] subParts = splitJsonArray(parts[i]);");
            sb.AppendLine("                        mat[i] = new String[subParts.length];");
            sb.AppendLine("                        for (int j = 0; j < subParts.length; j++) mat[i][j] = parseString(subParts[j]);");
            sb.AppendLine("                    }");
            sb.AppendLine("                    return mat;");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        if (List.class.isAssignableFrom(clazz)) {");
            sb.AppendLine("            String typeStr = (genericType != null) ? genericType.toString() : \"\";");
            sb.AppendLine("            String[] parts = splitJsonArray(raw);");
            sb.AppendLine("            if (typeStr.contains(\"List<List\") || typeStr.contains(\"List<java.util.List\")) {");
            sb.AppendLine("                List<List<Object>> res = new ArrayList<>();");
            sb.AppendLine("                for (String part : parts) {");
            sb.AppendLine("                    String[] subParts = splitJsonArray(part);");
            sb.AppendLine("                    List<Object> subList = new ArrayList<>();");
            sb.AppendLine("                    for (String sub : subParts) {");
            sb.AppendLine("                        if (typeStr.contains(\"Integer\")) {");
            sb.AppendLine("                            subList.add(Integer.parseInt(sub.trim()));");
            sb.AppendLine("                        } else {");
            sb.AppendLine("                            subList.add(parseString(sub));");
            sb.AppendLine("                        }");
            sb.AppendLine("                    }");
            sb.AppendLine("                    res.add(subList);");
            sb.AppendLine("                }");
            sb.AppendLine("                return res;");
            sb.AppendLine("            } else {");
            sb.AppendLine("                List<Object> res = new ArrayList<>();");
            sb.AppendLine("                for (String part : parts) {");
            sb.AppendLine("                    if (typeStr.contains(\"Integer\")) {");
            sb.AppendLine("                        res.add(Integer.parseInt(part.trim()));");
            sb.AppendLine("                    } else if (typeStr.contains(\"Double\")) {");
            sb.AppendLine("                        res.add(Double.parseDouble(part.trim()));");
            sb.AppendLine("                    } else if (typeStr.contains(\"Long\")) {");
            sb.AppendLine("                        res.add(Long.parseLong(part.trim()));");
            sb.AppendLine("                    } else {");
            sb.AppendLine("                        res.add(parseString(part));");
            sb.AppendLine("                    }");
            sb.AppendLine("                }");
            sb.AppendLine("                return res;");
            sb.AppendLine("            }");
            ...
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    private static String formatList(List<?> list) {");
            sb.AppendLine("        StringBuilder sb = new StringBuilder();");
            sb.AppendLine("        sb.append(\"[\");");
            sb.AppendLine("        for (int i = 0; i < list.size(); i++) {");
            sb.AppendLine("            Object obj = list.get(i);");
            sb.AppendLine("            if (obj instanceof List) {");
            sb.AppendLine("                sb.append(formatList((List<?>) obj));");
            sb.AppendLine("            } else if (obj instanceof String) {");
            sb.AppendLine("                sb.append(\"\\\"\").append(obj).append(\"\\\"\");");
            sb.AppendLine("            } else {");
            sb.AppendLine("                sb.append(obj);");
            sb.AppendLine("            }");
            sb.AppendLine("            if (i < list.size() - 1) sb.append(\",\");");
            sb.AppendLine("        }");
            sb.AppendLine("        sb.append(\"]\");");
            sb.AppendLine("        return sb.toString();");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    private static String serializeResult(Object obj) {");
            sb.AppendLine("        if (obj == null) return \"null\";");
            sb.AppendLine("        if (obj instanceof ListNode) return serializeListNode((ListNode) obj);");
            sb.AppendLine("        if (obj instanceof TreeNode) return serializeTreeNode((TreeNode) obj);");
            sb.AppendLine("        if (obj instanceof int[]) return Arrays.toString((int[]) obj).replace(\" \", \"\");");
            sb.AppendLine("        if (obj instanceof long[]) return Arrays.toString((long[]) obj).replace(\" \", \"\");");
            sb.AppendLine("        if (obj instanceof double[]) return Arrays.toString((double[]) obj).replace(\" \", \"\");");
            sb.AppendLine("        if (obj instanceof boolean[]) return Arrays.toString((boolean[]) obj).replace(\" \", \"\");");
            sb.AppendLine("        if (obj instanceof int[][]) return Arrays.deepToString((int[][]) obj).replace(\" \", \"\");");
            sb.AppendLine("        if (obj instanceof String[]) {");
            sb.AppendLine("            StringBuilder sb = new StringBuilder(\"[\");");
            sb.AppendLine("            String[] arr = (String[]) obj;");
            sb.AppendLine("            for (int i = 0; i < arr.length; i++) {");
            sb.AppendLine("                sb.append(\"\\\"\").append(arr[i]).append(\"\\\"\");");
            sb.AppendLine("                if (i < arr.length - 1) sb.append(\",\");");
            sb.AppendLine("            }");
            sb.AppendLine("            sb.append(\"]\");");
            sb.AppendLine("            return sb.toString();");
            sb.AppendLine("        }");
            sb.AppendLine("        if (obj instanceof String[][]) {");
            sb.AppendLine("            StringBuilder sb = new StringBuilder(\"[\");");
            sb.AppendLine("            String[][] mat = (String[][]) obj;");
            sb.AppendLine("            for (int i = 0; i < mat.length; i++) {");
            sb.AppendLine("                sb.append(\"[\");");
            sb.AppendLine("                for (int j = 0; j < mat[i].length; j++) {");
            sb.AppendLine("                    sb.append(\"\\\"\").append(mat[i][j]).append(\"\\\"\");");
            sb.AppendLine("                    if (j < mat[i].length - 1) sb.append(\",\");");
            sb.AppendLine("                }");
            sb.AppendLine("                sb.append(\"]\");");
            sb.AppendLine("                if (i < mat.length - 1) sb.append(\",\");");
            sb.AppendLine("            }");
            sb.AppendLine("            sb.append(\"]\");");
            sb.AppendLine("            return sb.toString();");
            sb.AppendLine("        }");
            sb.AppendLine("        if (obj instanceof char[]) {");
            sb.AppendLine("            StringBuilder sb = new StringBuilder(\"[\");");
            sb.AppendLine("            char[] arr = (char[]) obj;");
            sb.AppendLine("            for (int i = 0; i < arr.length; i++) {");
            sb.AppendLine("                sb.append(\"\\\"\").append(arr[i]).append(\"\\\"\");");
            sb.AppendLine("                if (i < arr.length - 1) sb.append(\",\");");
            sb.AppendLine("            }");
            ...
            sb.AppendLine("        if (obj instanceof char[][]) {");
            sb.AppendLine("            StringBuilder sb = new StringBuilder(\"[\");");
            sb.AppendLine("            char[][] mat = (char[][]) obj;");
            sb.AppendLine("            for (int i = 0; i < mat.length; i++) {");
            sb.AppendLine("                sb.append(\"[\");");
            sb.AppendLine("                for (int j = 0; j < mat[i].length; j++) {");
            sb.AppendLine("                    sb.append(\"\\\"\").append(mat[i][j]).append(\"\\\"\");");
            ...
            sb.AppendLine("            return sb.toString();");
            sb.AppendLine("        }");
            sb.AppendLine("        if (obj instanceof List) return formatList((List<?>) obj);");
            sb.AppendLine("        return String.valueOf(obj);");
            sb.AppendLine("    }");
            sb.AppendLine();
            ...
            sb.AppendLine($"            for (Method m : Solution.class.getDeclaredMethods()) {{");
            sb.AppendLine($"                if (m.getName().equals(\"{methodName}\") && !Modifier.isStatic(m.getModifiers())) {{");
            sb.AppendLine("                    targetMethod = m;");
            sb.AppendLine("                    break;");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("            if (targetMethod == null) {");
            sb.AppendLine("                for (Method m : Solution.class.getDeclaredMethods()) {");
            sb.AppendLine("                    if (!Modifier.isStatic(m.getModifiers())) {");
            sb.AppendLine("                        targetMethod = m;");
            sb.AppendLine("                        break;");
            sb.AppendLine("                    }");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("            if (targetMethod == null) {");
            sb.AppendLine("                System.err.println(\"No solution method found.\");");
            sb.AppendLine("                System.exit(1);");
            sb.AppendLine("            }");
            sb.AppendLine();
            sb.AppendLine("            Class<?>[] paramClasses = targetMethod.getParameterTypes();");
            sb.AppendLine("            Type[] genericParamTypes = targetMethod.getGenericParameterTypes();");
            sb.AppendLine("            int P = paramClasses.length;");
            sb.AppendLine();
            ...
            sb.AppendLine("                    callArgs[i] = parseArg(lines[i], paramClasses[i], genericParamTypes[i]);");
            sb.AppendLine("                }");
            sb.AppendLine();
            sb.AppendLine("                Object res = targetMethod.invoke(sol, callArgs);");
            sb.AppendLine("                if (targetMethod.getReturnType() == void.class) {");
            sb.AppendLine("                    if (P > 0) {");
            sb.AppendLine("                        System.out.println(serializeResult(callArgs[0]));");
            sb.AppendLine("                    }");
            sb.AppendLine("                } else {");
            sb.AppendLine("                    System.out.println(serializeResult(res));");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("        } catch (Exception ex) {");
            sb.AppendLine("            Throwable cause = ex.getCause() != null ? ex.getCause() : ex;");
            sb.AppendLine("            cause.printStackTrace();");
            sb.AppendLine("            System.exit(1);");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine("}");
